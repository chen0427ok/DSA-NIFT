...
### 實驗 5：L3 圖譜可控生成增強（進行中）
- **基礎**：實驗 4（L1 詞典融合）不變。
- **唯一新增**：`augment_generate.py` 讀 L3 情感知識圖譜（`outputs/l3_graph.pkl`），對高/低 arousal
  等代表性不足的 VA 區間撈種子詞 → 請 `claude-opus-4-8` 生成符合該情緒強度的新住民第一人稱反思短文
  → 標上目標 VA（含 jitter）→ 併進 `train.csv`（`--append`，原始備份為 `train_base.csv`）。
- **目的**：直接補 arousal 分布壓縮——靠合成高/低喚醒樣本拉開 arousal 的訓練分布，期望提升 A_PCC。
- **結果**：⏳ 待 Colab 跑完填入。觀察重點：arousal 預測 std 是否更接近真實、A_PCC 是否突破 0.43。
