...
- **成因判讀（這是論文的核心分析，見 §5.3）**：我們整個模型選擇流程是對著**一個 200 篇的 leaderboard 標量**做的。
  A_PCC 在 n=200 上的抽樣誤差本來就大（±0.06 量級），而我們用 13 次提交在這個雜訊上挑第一名 →
  **典型的 selection overfitting**。E19 相對 E4 的「+0.026 A_PCC」很可能大半是雜訊，不是真實增益。
- 佐證：內部 dev（DSA-MST 253 篇）上 5 個 seed 的 A_PCC 落在 **0.604–0.616**（純 seed 雜訊帶就有 0.012 寬），
  而我們在 val 上據以決策的差距經常小於這個量級。
...
這是 test 結果逼出來的、也是最有價值的一節。
在 200 篇的 official validation 上做 13 次提交挑最佳，得到 A_PCC 0.452；
到 1,100 篇 test 上只剩 0.357，而 valence 幾乎不動（0.869 → 0.866）。
**arousal 的評估變異遠大於 valence**，小驗證集上的 arousal PCC 差距（±0.03）基本不可信。
建議：此類任務應報告 seed 變異與信賴區間，並避免用單一小型 leaderboard 標量做多輪選擇。
