...
## 0. 最終官方成績（Test Set，1,100 篇）

**正式提交：E19（`e19_source_aware`）。** 評測結束後網站仍可提交（每日 10 次額度），
因此我們補測了 E4，得到本專案最關鍵的一組對照。

| 系統 | V_MAE ↓ | V_PCC ↑ | A_MAE ↓ | A_PCC ↑ |
|---|---|---|---|---|
| **E4**（10 維 L1）※非提交 | **0.61** 🟢 | **0.87** 🟢 | 0.94 | **0.37** 🟢 |
| **E19**（31 維 + source-aware）**← 實際提交** | 0.6200 | 0.8663 | **0.9259** 🟢 | 0.3566 |

### 💥 核心發現：我們選錯了，而且 arousal 的排序在兩個集合之間**翻轉**
...
- **排序反轉。** 我們在 val 上據以選擇 E19 的那個優勢，到 test 不但消失、還倒過來。
- **E4 在 test 上四指標贏三項**（V_MAE、V_PCC、A_PCC），E19 只保住 A_MAE。
  ⇒ **若當初提交 E4，官方 mean rank 會更好。**
- E19 在 val 上號稱的「arousal 雙贏」（A_MAE 與 A_PCC 同時優於 E4），
  在 test 上只剩 A_MAE 一半成立。
- ⚠️ **但兩邊的差距都遠小於噪聲**（test 的 A_PCC 95% CI 寬度約 0.103，而差距只有 0.013）。
  正確的結論不是「E4 比較好」，而是 **「這兩個系統從頭到尾就分不出高下，
  我們卻在 val 上把 0.026 的雜訊當成訊號、據此做了決策」**。

> 這是整篇論文最有力的一段：**不是模型失敗，是模型選擇的方法論失敗**，且我們有兩個集合的直接對照可證。

### validation → test 的完整落差
...
四個環環相扣的證據：
1. **排序反轉（最強證據）**：A_PCC 上 val 是 E19（0.452）> E4（0.426），
   test 卻是 E4（0.37）> E19（0.3566）。**我們據以決策的偏好方向在更大的集合上倒過來。**
2. **落差本身**：E19 的 A_PCC val 0.452 → test 0.357（−0.095），而 V_PCC 0.869 → 0.866（−0.003）。
3. **噪聲量級**：bootstrap 顯示 n=200 時 A_PCC 的 95% CI 寬達 **0.193**（valence 只有 0.083），
   而我們做決策依據的差距只有 0.026。**決策差距比噪聲小 7.4 倍。**
   Fisher-z 也顯示 val 的 A_PCC 區間 [0.334, 0.556] 與 test 的 [0.304, 0.408] **重疊**。
4. **排除替代解釋**：模型在 test 上的預測分布與 val 幾乎相同（arousal std 0.729 vs 0.730），
   所以不是模型漂移；**官方 test 上 5 個 seed 的 A_PCC = 0.372 ± 0.004（全距 0.37–0.38）**，
   所以不是訓練隨機性。**只剩「評估集太小」這一個解釋。**
   （這組 seed 變異原本是 dev bootstrap 的代理估計 0.0055，現已用官方 test 實測取代——
   見 `docs/test_results.md`；leaderboard 只到 2 位小數，故 0.004 為上界。）

⚠️ **論述紀律**：不要寫成「E4 比 E19 好」——test 上 0.013 的差距同樣在噪聲裡。
正確的主張是**「兩者不可區分，而我們把不可區分當成了可區分」**。
