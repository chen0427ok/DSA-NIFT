...
| F2 | s1 | 0.63 | 0.86 | 0.96 | 0.38 |
| F2 | s2 | 0.63 | 0.87 | 1.07 | **0.40** |
| F2 | s42 | 0.63 | 0.87 | 1.19 | 0.38 |
...
| F（1 seed） | 0.38 | 1.08 |
| **F2（圖+錨定+真實長度）** | **0.387±0.012** | 1.073±0.115 |
| **無增強 baseline（matched batch 32）** | **0.370±0.000** | **0.923±0.040** |
...
1. **種子詞有效，但 VA 過濾與圖結構都無可測貢獻**：N→A +0.014；A→C 與 C→E 都只有 +0.003，
   ...
   C（查表）→ E（圖擴散）只 +0.003。圖的連貫度約 14.2 倍、arousal spread 更大，
   **都沒有轉成相應幅度的排序提升**。
   增益主要來自「有任意種子詞」（N→A）與「風格錨定+長度」（E→F2），
   VA 過濾（A→C）與圖結構（C→E）皆只有 +0.003。
2. **F2 呈現明確 trade-off**：相對 matched 無增強 baseline，A_PCC 由 0.370 提高到 0.387，
   而 A_MAE 由 0.923 惡化到 1.073。較高排序關聯沒有形成更好的 PCC--MAE 組合。
...
1. **圖結構對 A_PCC 的改變很小**：C 0.370 vs E 0.373，差 0.003。
   儘管 E 的種子連貫度約為 C 的 14.2 倍、test arousal spread 也更大（0.85 vs 0.82），
   **這些都沒有轉成 arousal 排序的提升**。
2. **F2 提高 A_PCC**：matched batch-32 下由無增強 0.370 提高到 0.387。
3. **增強同時傷 A_MAE**：E/F2 為 1.057/1.073，無增強為 0.923（+0.133/+0.150）。
   ⇒ **實驗 5「A_PCC 撐開但 A_MAE 惡化」的現象在 n=1,100 上完整重現。**
